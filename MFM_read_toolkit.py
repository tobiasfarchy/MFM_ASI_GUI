...
def clean_peaks(data_raw, quant = 0.7, dist: int = 8):
    ns = np.arange(len(data_raw))
    fit = np.polyfit(ns, data_raw, deg = 1)
    data = data_raw - fit[0]*ns - fit[1]
    peak_i, peak_h = find_peaks(data, height = (np.quantile(data, quant), np.max(data) + 1.), distance = dist)
...
def calc_spacing(peak_i):
    space = [peak_i[i+1] - peak_i[i] for i in range(len(peak_i) - 1)]
    return space


def square_lattice_peaks(AFM_input, x0, y0):
    # Collect peaks for x0 and y0
    row_raw = AFM_input[y0]
    x0_i = clean_peaks(data_raw = row_raw, quant = 0.8)
    
    col_raw = [AFM_input[i][x0] for i in range(len(AFM_input))]
    y0_i = clean_peaks(data_raw = col_raw, quant = 0.8)
    ...
    for y in y_gaps:
        if y != y0:
            ...
            x_i = clean_peaks(data_raw = row_raw) + max(0, x0_i[0] - int(x0_space[0]/2))
    ...
    for x in x_gaps:
        if x != x0:
            ...
            y_i = clean_peaks(data_raw = col_raw) + max(0, y0_i[0] - int(y0_space[0]/2))
            if len(y_i) == len(y0_i):
                y_ypeaks.append(y_i)
                x_yp.append(x)
        else:
...
# def square_lattice_vertices(AFM_input, MFM_input, x0, y0):
#     ((x_xpeaks, y_xp), (x_yp, y_ypeaks)) = square_lattice_peaks(AFM_input, x0, y0)
...
def square_lattice_bars(AFM_input, MFM_input, x0, y0):
    ((x_xpeaks, y_xp), (x_yp, y_ypeaks)) = square_lattice_peaks(AFM_input, x0, y0)
    ...
    xbars = np.zeros((vrtx_coords.shape[0], vrtx_coords.shape[1] - 1, int(ly/2), lx)) #iceil
    ybars = np.zeros((vrtx_coords.shape[0] - 1, vrtx_coords.shape[1], ly, int(lx/2))) #iceil
